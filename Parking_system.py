...
date_year = xTime.year 
date_month= xTime.month
date_day=xTime.day
...
def display_vehicle3():
    ...
    if not registered_plates:
        ...
        input("Please Press enter to continue...") # allows the user a break
    else:
        ...
        my_file.close()
        print(f'A list of registered vehicles for {date_day}-{date_month}-{date_year}') # Can we get a function/obj/class(whatever) to display the date for that day? or should this be tied to the date the text file was created or last edited?
        ...
        my_file.write("====================\n")
        my_file.close()
        print(f'Daily parking summary for {date_day}-{date_month}-{date_year} ') # Can we get a function/obj/class(whatever) to display the date for that day? or should this be tied to the date the text file was created or last edited?
...
def display_Charges4():
    ...
    for i in range(len(registered_plates)):
        ...
        cc = registered_CC[i]
        my_file.write(f"{str(plate):>12}{str(cc):>22}{charge:>15}\n")# to format data types string you must specify str before the variable example{str(item):>11.2f} ////////  {total_interest:>14.2f} is meant to format integers.
